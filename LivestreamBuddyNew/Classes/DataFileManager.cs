...
namespace LiveStreamBuddy.Classes
{
	public static class DataFileManager
	{
		...
		{
			var favoriteChannels = new List<string>();
		...
		{
			bool exists =
				GetFavoriteChannels().Any(channel => string.Compare(newChannel, channel, StringComparison.OrdinalIgnoreCase) == 0);
		...
		public static void RemoveFavoriteChannel(string channelName)
		{
			List<string> channels =
				GetFavoriteChannels()
					.Where(channel => string.Compare(channelName, channel, StringComparison.OrdinalIgnoreCase) != 0)
					.ToList();
					...
					while ((line = reader.ReadLine()) != null)
					{
						...
						switch (values[0])
						{
							...
							case "accesstoken":
								...
								break;
							case "user_read":
								user.AccessTokens.Add(UserScope.UserRead,
									new AccessToken {Token = values[1], Retrieved = new DateTime(Convert.ToInt64(values[2]))});
								break;
							case "user_blocks_edit":
								user.AccessTokens.Add(UserScope.UserBlocksEdit,
									new AccessToken {Token = values[1], Retrieved = new DateTime(Convert.ToInt64(values[2]))});
								break;
							case "user_blocks_read":
								user.AccessTokens.Add(UserScope.UserBlocksRead,
									new AccessToken {Token = values[1], Retrieved = new DateTime(Convert.ToInt64(values[2]))});
								break;
							case "user_follows_edit":
								user.AccessTokens.Add(UserScope.UserFollowsEdit,
									new AccessToken {Token = values[1], Retrieved = new DateTime(Convert.ToInt64(values[2]))});
								break;
							case "channel_read":
								user.AccessTokens.Add(UserScope.ChannelRead,
									new AccessToken {Token = values[1], Retrieved = new DateTime(Convert.ToInt64(values[2]))});
								break;
							case "channel_editor":
								user.AccessTokens.Add(UserScope.ChannelEditor,
									new AccessToken {Token = values[1], Retrieved = new DateTime(Convert.ToInt64(values[2]))});
								break;
							case "channel_commercial":
								user.AccessTokens.Add(UserScope.ChannelCommercial,
									new AccessToken {Token = values[1], Retrieved = new DateTime(Convert.ToInt64(values[2]))});
								break;
							case "channel_stream":
								user.AccessTokens.Add(UserScope.ChannelStream,
									new AccessToken {Token = values[1], Retrieved = new DateTime(Convert.ToInt64(values[2]))});
								break;
							case "channel_subscriptions":
								user.AccessTokens.Add(UserScope.ChannelSubscriptions,
									new AccessToken {Token = values[1], Retrieved = new DateTime(Convert.ToInt64(values[2]))});
								break;
							case "channel_check_subscriptions":
								user.AccessTokens.Add(UserScope.ChannelCheckSubscription,
									new AccessToken {Token = values[1], Retrieved = new DateTime(Convert.ToInt64(values[2]))});
								break;
							case "chat_login":
								user.AccessTokens.Add(UserScope.ChatLogin,
									new AccessToken {Token = values[1], Retrieved = new DateTime(Convert.ToInt64(values[2]))});
								break;
						}
					}
		...
		public static void SetUser(User user)
		{
			if (user != null)
			{
				using (StreamWriter writer = File.CreateText(identitiesFileName))
				{
					...
					foreach (var kvp in user.AccessTokens)
					{
						writer.WriteLine(string.Format("{0} {1} {2}", EnumHelper.GetUserScope(kvp.Key), kvp.Value.Token,
							kvp.Value.Retrieved.Ticks.ToString()));
					}
				}
			}
		}
